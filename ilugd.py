...
async def greet_chat_members(
    ...
    if not was_member and is_member:
        await update.effective_chat.send_message(
            "Hello {member_name}! Welcome to Ilug-D .let's start with introduction, was added by {cause_name}.",
...
async def website(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = cpnfig["BOT"]["website"]
    await update.effective_message.reply_text(text)


async def invitelink(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = config["BOT"]["invite_link"]
    await update.effective_message.reply_text(text)


async def mailinglist(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = config["BOT"]["mailinglist"]
    await update.effective_message.reply_text(text)


async def twitter(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = config["BOT"]["twitter"]
    await update.effective_message.reply_text(text)


async def meetup(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = config["BOT"]["meetuplink"]
    await update.effective_message.reply_text(text)


async def github(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = config["BOT"]["github"]
    await update.effective_message.reply_text(text)
...
def main() -> None:
    ...
    application = Application.builder().token(tokenid).build()
    ...
    application.add_handler(CommandHandler("invite", invitelink))
    application.add_handler(
        CommandHandler("mailinglist", mailinglist)
